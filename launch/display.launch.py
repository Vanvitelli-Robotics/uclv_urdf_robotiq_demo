...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument('urdf_package',
                                        default_value='uclv_urdf_robotiq_demo',
                                        description='The package where the robot description is located'))
    ld.add_action(DeclareLaunchArgument('urdf_package_path',
                                        default_value='urdf/robotiq_arg2f_140.urdf.xacro',
                                        description='The path to the robot description relative to the package root'))
    ########################################

    #### Argument: .rviz config file location #####
    # Find the path of the package 'uclv_urdf_robotiq_demo'
    this_package = FindPackageShare('uclv_urdf_robotiq_demo')
    default_rviz_config_path = PathJoinSubstitution([this_package, 'config', 'urdf.rviz'])
